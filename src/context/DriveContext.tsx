...
interface DriveContextType {
  ...
  shareModalTarget: ShareModalTarget | null;
  setShareModalTarget: (target: ShareModalTarget | null) => void;
  ...
  deleteModalTarget: DeleteModalTarget | null;
  setDeleteModalTarget: (target: DeleteModalTarget | null) => void;
  ...
  deleteMultipleItems: (fileIds: string[], folderIds?: string[]) => Promise<void>;
  toggleStarFile: (id: string) => Promise<void>;
  downloadFile: (file: FileItem) => void;
  // Trash actions
  restoreItem: (type: 'file' | 'folder', id: string) => Promise<void>;
  restoreMultipleItems: (fileIds: string[], folderIds?: string[]) => Promise<void>;
  permanentDeleteItem: (type: 'file' | 'folder', id: string) => Promise<void>;
  deleteMultiplePermanently: (fileIds: string[], folderIds?: string[]) => Promise<void>;
...
export const DriveProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (searchQuery.trim() !== '' || mediaTypeFilter !== 'all') {
        ...
        // Sorting helper for folders
        const sortFolders = (list: typeof loadedFolders) => {
        ...
        // Sorting helper for files
        const sortFiles = (list: typeof loadedFiles) => {
          return [...list].sort((a, b) => {
            let cmp = 0;
            if (sortField === 'name') cmp = a.name.localeCompare(b.name);
            else if (sortField === 'date') cmp = new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime();
        ...
        let loadedFolders = [...sharedRes.folders].sort((a, b) => {
          let cmp = sortField === 'date'
            ? new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime()
            : a.name.localeCompare(b.name);
          return sortOrder === 'desc' ? -cmp : cmp;
        });
        let loadedFiles = [...sharedRes.files].sort((a, b) => {
          let cmp = 0;
          if (sortField === 'name') cmp = a.name.localeCompare(b.name);
          else if (sortField === 'date') cmp = new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime();
          else if (sortField === 'size') cmp = a.size - b.size;
        ...
        let loadedFolders = [...starredRes.folders].sort((a, b) => {
          let cmp = sortField === 'date'
            ? new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime()
            : a.name.localeCompare(b.name);
          return sortOrder === 'desc' ? -cmp : cmp;
        });
        let loadedFiles = [...starredRes.files].sort((a, b) => {
          let cmp = 0;
          if (sortField === 'name') cmp = a.name.localeCompare(b.name);
          else if (sortField === 'date') cmp = new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime();
          else if (sortField === 'size') cmp = a.size - b.size;
        ...
        let loadedFiles = [...recentRes.files].sort((a, b) => {
          let cmp = 0;
          if (sortField === 'name') cmp = a.name.localeCompare(b.name);
          else if (sortField === 'date') cmp = new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime();
          else if (sortField === 'size') cmp = a.size - b.size;
        ...
        let loadedFolders = [...trashRes.folders].sort((a, b) => {
          let cmp = sortField === 'date'
            ? new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime()
            : a.name.localeCompare(b.name);
          return sortOrder === 'desc' ? -cmp : cmp;
        });
        let loadedFiles = [...trashRes.files].sort((a, b) => {
          let cmp = 0;
          if (sortField === 'name') cmp = a.name.localeCompare(b.name);
          else if (sortField === 'date') cmp = new Date(a.updatedAt || a.createdAt).getTime() - new Date(b.updatedAt || b.createdAt).getTime();
          else if (sortField === 'size') cmp = a.size - b.size;
        ...
        setTrashFiles(loadedFiles);
        setTrashFolders(loadedFolders);
  ...
  const deleteMultipleItems = async (fileIds: string[], folderIds: string[] = []) => {
    const promises: Promise<any>[] = [];
    for (const fId of fileIds) {
      promises.push(api.deleteFile(fId));
    }
    for (const foldId of folderIds) {
      promises.push(api.deleteFolder(foldId));
    }
  ...
  const restoreMultipleItems = async (fileIds: string[], folderIds: string[] = []) => {
    const promises: Promise<any>[] = [];
    for (const fId of fileIds) {
      promises.push(api.restoreItem('file', fId));
    }
    for (const foldId of folderIds) {
      promises.push(api.restoreItem('folder', foldId));
    }
    await Promise.all(promises);
    clearSelection();
    refresh();
  ...
  const deleteMultiplePermanently = async (fileIds: string[], folderIds: string[] = []) => {
    const promises: Promise<any>[] = [];
    for (const fId of fileIds) {
      promises.push(api.deletePermanently('file', fId));
    }
    for (const foldId of folderIds) {
      promises.push(api.deletePermanently('folder', foldId));
    }
    await Promise.all(promises);
    clearSelection();
    refresh();
