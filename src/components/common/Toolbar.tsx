...
export const Toolbar: React.FC = () => {
  ...
  const getSortFieldLabel = (field: SortField) => {
    if (field === 'date') return 'Last modified';
    if (field === 'size') return 'File size';
    return 'Name';
  };

  const getSortOrderBadge = (field: SortField, order: 'asc' | 'desc') => {
    if (field === 'date') return order === 'asc' ? 'Oldest first' : 'Newest first';
    if (field === 'size') return order === 'asc' ? 'Smallest' : 'Largest';
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {selectedCount > 0 && !isTrashTab && (
          <div className="flex items-center gap-1 border-r border-[#E5E5DF] pr-2 mr-1 animate-in fade-in duration-100">
            {/* Share action (for 1 or multiple selected items) */}
            <button
              onClick={() => {
                if (singleFile) {
                  setShareModalTarget({ type: 'file', id: singleFile.id, name: singleFile.name });
                } else if (singleFolder) {
                  setShareModalTarget({ type: 'folder', id: singleFolder.id, name: singleFolder.name });
                } else {
                  const label =
              ...
              className="p-2 text-[#71716A] hover:text-[#5A5A40] hover:bg-[#F5F5F0] rounded-full transition-colors cursor-pointer"
              title={selectedCount > 1 ? `Share ${selectedCount} items` : 'Share'}
            >
            ...
            <button
              onClick={() => {
                if (selectedFileIds.length === 1 && selectedFolderIds.length === 0 && singleFile) {
                  setDeleteModalTarget({ type: 'file', id: singleFile.id, name: singleFile.name });
                } else if (selectedFolderIds.length === 1 && selectedFileIds.length === 0 && singleFolder) {
                  setDeleteModalTarget({ type: 'folder', id: singleFolder.id, name: singleFolder.name });
                } else if (selectedCount > 0) {
                  const label = selectedFileIds.length > 0 && selectedFolderIds.length === 0
              ...
              className="p-2 text-[#71716A] hover:text-rose-600 hover:bg-rose-50 rounded-full transition-colors cursor-pointer"
              title="Move to Trash"
            >
        ...
        {isTrashTab && selectedCount > 0 && (
          <div className="flex items-center gap-1 border-r border-[#E5E5DF] pr-2 mr-1">
            <button
              onClick={() => {
                if (selectedFileIds.length === 1 && selectedFolderIds.length === 0 && singleFile) {
                  restoreItem('file', singleFile.id);
                } else if (selectedFolderIds.length === 1 && selectedFileIds.length === 0 && singleFolder) {
                  restoreItem('folder', singleFolder.id);
                } else if (selectedCount > 0) {
                  restoreMultipleItems([...selectedFileIds], [...selectedFolderIds]);
                }
              }}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#5A5A40]/10 text-[#5A5A40] hover:bg-[#5A5A40]/20 rounded-full text-xs font-semibold transition-colors cursor-pointer"
            >
            ...
            <button
              onClick={() => {
                if (selectedFileIds.length === 1 && selectedFolderIds.length === 0 && singleFile) {
                  setDeleteModalTarget({ type: 'file', id: singleFile.id, name: singleFile.name, isPermanent: true });
                } else if (selectedFolderIds.length === 1 && selectedFileIds.length === 0 && singleFolder) {
                  setDeleteModalTarget({ type: 'folder', id: singleFolder.id, name: singleFolder.name, isPermanent: true });
                } else if (selectedCount > 0) {
                  const label = selectedFileIds.length > 0 && selectedFolderIds.length === 0
                    ? `${selectedFileIds.length} file${selectedFileIds.length > 1 ? 's' : ''}`
                    : `${selectedCount} item${selectedCount > 1 ? 's' : ''}`;
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-full text-xs font-semibold transition-colors cursor-pointer"
            >
        ...
        <div className="relative" ref={sortMenuRef}>
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-[#2D2D2A] bg-white hover:bg-[#F5F5F0] rounded-full transition-colors border border-[#E5E5DF] shadow-2xs"
            title="Sort options"
          >
            ...
            <span className="text-[10px] text-[#8E8E8A] font-normal">
              ({getSortOrderBadge(sortField, sortOrder)})
            </span>
          </button>

          {isSortOpen && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl shadow-xl border border-[#E5E5DF] py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150 text-xs">
              ...
              {(['name', 'date', 'size'] as SortField[]).map((field) => (
                ...
                >
                  ...
                  {sortField === field && (
                    <span className="text-[10px] uppercase font-bold text-[#5A5A40] bg-[#5A5A40]/15 px-1.5 py-0.5 rounded-full">
                      {sortOrder}
                    </span>
                  )}
                </button>
