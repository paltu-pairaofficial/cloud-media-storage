...
export const FileCard: React.FC<FileCardProps> = ({ file, isTrash = false }) => {
  ...
  return (
    ...
    >
      ...
      {/* Floating Selection checkbox on top-left */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          toggleSelectFile(file.id, true);
        }}
        className={`absolute top-4 left-4 z-10 w-6 h-6 rounded-lg flex items-center justify-center transition-all cursor-pointer ${
          isSelected
            ? 'bg-[#5A5A40] text-white shadow-xs opacity-100'
            : selectedFileIds.length > 0
            ? 'bg-white/90 border border-[#D1D1CB] text-transparent hover:border-[#5A5A40] opacity-80 hover:opacity-100'
            : 'bg-white/90 border border-[#D1D1CB] text-transparent hover:border-[#5A5A40] opacity-0 group-hover:opacity-100'
        }`}
