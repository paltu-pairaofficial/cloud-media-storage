...
export const DeleteModal: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    setError('');
    try {
      if (deleteModalTarget.type === 'multiple') {
        const fileIds = deleteModalTarget.fileIds || [];
        const folderIds = deleteModalTarget.folderIds || [];
        if (isPermanent) {
          await deleteMultiplePermanently(fileIds, folderIds);
        } else {
          await deleteMultipleItems(fileIds, folderIds);
        }
      } else if (isPermanent) {
