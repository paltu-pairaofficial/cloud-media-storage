...
export const api = {
  ...
  async forgotPassword(email: string): Promise<{ message: string; resetToken?: string; resetLink?: string; email?: string }> {
  ...
  async resetPassword(token: string, newPassword: string): Promise<{ message: string; email?: string }> {
  ...
  async shareMultipleResources(data: {
    fileIds?: string[];
    folderIds?: string[];
    email: string;
    role: 'viewer' | 'editor';
  }): Promise<{ message: string; sharedCount: number; recipient: any }> {
    const res = await fetch(`${API_BASE}/shares/bulk`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify(data),
    });
